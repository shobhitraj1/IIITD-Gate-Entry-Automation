...
votes_thresh = 5
merge_time_window = 10  # 10 second window for merging identities
...
def main():
    ...
    seens = []
    dones = []
    track_timestamps = []  # Store timestamp for each track
    merged_tracks = {}     # Track mapping: old_id -> new_id
    active_identities = {}  # Map identity to last seen track_id and timestamp
    recognition_cache = {}  # Cache for face recognition

    out_vid_name = f"{os.path.basename(vid_name).split('.')[0]}_merge.mp4"
    ...
    current_time = 0.0  # Track video time in seconds
    ...
    print(f"Track merging enabled with {merge_time_window} second window")
    ...
    while True:
        ...
        current_time = raw_frame_idx / orig_fps
        ...
        for track in online_tracks:
            ...
            orig_track_id = track.track_id
            
            # Apply track merging - check if this track ID has been merged into another
            track_id = orig_track_id
            while track_id in merged_tracks:
                track_id = merged_tracks[track_id]
            ...
            while track_id >= len(preds):
                ...
                seens.append(False)
                dones.append(False)
                track_timestamps.append(0.0)

            if prediction != "Unknown":
                preds[track_id-1][prediction] += 1
                
                # Identity merging logic
                if preds[track_id-1][prediction] >= 3:  # Require a minimum of 3 votes for confidence
                    majority_vote = max(preds[track_id-1], key=preds[track_id-1].get)
                    
                    # Check if this identity exists in active_identities and is within time window
                    if majority_vote in active_identities:
                        prev_id, prev_time = active_identities[majority_vote]
                        
                        # Only merge if this is a different track ID and within time window
                        if prev_id != track_id and (current_time - prev_time) <= merge_time_window:
                            # Determine the target ID to keep (always use the lower ID number)
                            target_id = min(prev_id, track_id)
                            source_id = max(prev_id, track_id)
                            
                            # Only merge if not already merged
                            if source_id not in merged_tracks:
                                print(f"Merging track {source_id} into {target_id} (identity: {majority_vote})")
                                
                                # Update the merged_tracks dictionary
                                merged_tracks[source_id] = target_id
                                
                                # Merge vote counts - add all votes from source to target
                                source_idx = source_id - 1
                                target_idx = target_id - 1
                                
                                # Merge all predictions from source to target
                                for pred_class, count in preds[source_idx].items():
                                    preds[target_idx][pred_class] += count
                                    
                                # Print vote counts before and after merging for verification
                                target_votes = sum(preds[target_idx].values())
                                print(f"After merging: ID {target_id} has {target_votes} total votes")
                                print(f"  Vote distribution: {dict(preds[target_idx])}")
                                
                                # Update tracking timeline
                                if seens[source_idx]:
                                    seens[target_idx] = True
                                    
                                    # Update first/last frame tracking
                                    if first_frame[source_idx] < first_frame[target_idx] or first_frame[target_idx] == 0:
                                        first_frame[target_idx] = first_frame[source_idx]
                                        
                                    if last_frame[source_idx] > last_frame[target_idx]:
                                        last_frame[target_idx] = last_frame[source_idx]
                            
                            # Update track_id to the target for the rest of this iteration
                            track_id = target_id
                    
                    # Update active_identities with current track
                    active_identities[majority_vote] = (track_id, current_time)

            # Draw bounding box and label
            cv2.rectangle(frame, (x1, y1), (x2, y2), (0, 255, 0), 2)
            
            # Show original and merged track IDs if they differ
            id_text = f'ID:{int(track_id)}'
            if track_id != orig_track_id:
                id_text += f' (was {orig_track_id})'
                
            # Display total votes and majority prediction
            total_votes = sum(preds[track_id-1].values())
            if total_votes > 0:
                majority = max(preds[track_id-1], key=preds[track_id-1].get)
                majority_votes = preds[track_id-1][majority]
                
                cv2.putText(
                    frame,
                    f'{id_text} {prediction} ({majority_votes}/{total_votes}) {score:.2f}',
                    (x1, y1-10),
            ...
            if not seens[track_id-1]:
                ...
                track_timestamps[track_id-1] = current_time

            last_frame[track_id-1] = len(tracked_objects)  # Update last seen frame
            track_timestamps[track_id-1] = current_time    # Update timestamp
        ...
        if run_detection:
            for i in range(len(preds)):
                # Skip if this track ID is in the merged_tracks dictionary keys
                if i+1 in merged_tracks:
                    continue
                    
                if not dones[i] and i < len(last_frame) and last_frame[i] < len(tracked_objects):
                    # Mark track as done if not seen for track_buffer frames
                    if raw_frame_idx - tracked_objects[last_frame[i]][0] > args.track_buffer * skip_frames:
                        dones[i] = True
                        print(f"ID {i+1} done (timeout)")
                        
                    # Check if track has left the frame
                    ...
                    # Report person exit if enough votes
                    if dones[i] and len(preds[i]) > 0:
                        ...
                        if votes >= votes_thresh:
                            ...
                            print(f"  Vote distribution: {dict(preds[i])}")

        # Clean up outdated active_identities entries
        identities_to_remove = []
        for identity, (id_val, timestamp) in active_identities.items():
            if current_time - timestamp > merge_time_window:
                identities_to_remove.append(identity)
        
        for identity in identities_to_remove:
            active_identities.pop(identity)
        ...
        if processed_frame_idx % 10 == 0:
            ...
            print(f"Active track merges: {len(merged_tracks)}")
            
            # Print tracking stats for active tracks
            if len(preds) > 0 and processed_frame_idx % 30 == 0:
                print("Current active tracks with vote counts:")
                for i in range(len(preds)):
                    if i+1 not in merged_tracks and len(preds[i]) > 0 and not dones[i]:
                        majority = max(preds[i], key=preds[i].get)
                        print(f"  ID {i+1}: Recognized as {majority} - Votes: {dict(preds[i])}")

    # Final processing for remaining tracks
    print("Video Mode Over")
    for i in range(len(preds)):
        # Skip reporting for merged tracks (source tracks, not target tracks)
        if i+1 in merged_tracks:
            continue
    ...
    print(f"Total track merges performed: {len(merged_tracks)}")
