...
class DSFD(nn.Module):
    ...
    def test_forward(self, x):
        ...
        if self.input_size != size:
            ...
            with torch.no_grad():
                priorbox = PriorBox(size, features_maps, cfg, pal=2)
                self.register_buffer('priors_pal2', Variable(priorbox.forward()))
    ...
    def forward(self, x, x_light, I, I_light):
        ...
        if not hasattr(self, 'priors_pal2'):
            priorbox = PriorBox(size, features_maps, cfg, pal=2)
            self.register_buffer('priors_pal2', Variable(priorbox.forward(), volatile=True))
